...
def rotate_image(image, coords_pairs):
    if random.random() < 0.0:
...
def apply_augmentations(image, coords_pairs):
    ...
    image, adjusted_coords = rotate_image(image, adjusted_coords)
    ...
    for image_file in sorted(os.listdir(image_dir)):
        ...
        try:
            ...
            if class_id not in ['0', '1']:
                continue
            available_images[class_id].append(image_file)
        except Exception as e:
            print(f"Erro ao processar {image_file}: {e}")

    for class_id, files in available_images.items():
        ...
        for i, image_file in enumerate(files):
            ...
            try:
                ...
                for copy_num in range(copies):
                    ...
                    if class_count[class_id] >= target_per_class:
                        break
            except Exception as e:
                print(f"Erro ao processar {image_file}: {e}")

    for class_id in class_count:
        print(f"Classe {class_id}: {class_count[class_id]} imagens aumentadas (target: {target_per_class})")
